...
import { runRobustWalkForwardValidation } from "@/lib/backtest/run-robust-walk-forward";
...
export async function POST(
  ...
  try {
    ...
    const result =
      await runRobustWalkForwardValidation({
        stockCodes:
          getStringArray(
            body.stockCodes,
          ),
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Robust Walk-Forward 검증 중 오류가 발생했습니다.",
      },
      {
        status: 500,
      },
