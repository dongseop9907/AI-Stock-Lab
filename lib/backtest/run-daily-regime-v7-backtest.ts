...
function calculateMarketRegime(
   ...
   * 나머지는 NEUTRAL.
   *
   * 과거 20일 정보가 부족하면 UNKNOWN으로 두고,
   * v5에서는 UNKNOWN을 차단하지 않는다.
   */
  if (
...
export async function runDailyRegimeV7Backtest(
  input:
  ...
  /*
   * 20거래일 시장 국면을 첫 테스트일부터 계산하려면
   * 테스트 시작일 이전 데이터가 필요하다.
   *
   * 45 calendar days를 warm-up으로 읽되,
   * warm-up 구간에서는 거래하지 않는다.
   */
      ...
      .insert({
        ...
        config: {
          ...
          regimeDefinition: {
            ...
            bull:
              "breadth20 >= 0.60 && avgReturn20 > 0",

            bear:
              "breadth20 <= 0.40 && avgReturn20 < 0",
  ...
  try {
    ...
    const closePosition = (
      ...
      trades.push({
        ...
        metadata: {
          ...
          marketRegimeAtSignal:
            position.marketRegime ??
            "UNKNOWN",

          marketBreadth20AtSignal:
            position.marketBreadth20 ??
            null,

          marketReturn20AtSignal:
            position.marketReturn20 ??
            null,
        },
      });
    ...
    for (
      ...
      /*
       * --------------------------------------------------------
       * 6. 오늘 종가까지의 정보로 시장 국면 계산
       *
       * signalDate = 오늘
       * 실제 체결 = 다음 거래일 시가
       *
       * 따라서 미래 데이터는 사용하지 않는다.
       * --------------------------------------------------------
       */
      const marketRegime =
        ...
        pendingEntries.set(
          bar.stock_code,
          {
            signal: {
              ...
              marketBreadth20:
                marketRegime.breadth20,

              marketReturn20:
                marketRegime.avgReturn20,
            },
          },
        );
      }
