...
async function resolveLatestMarketDate(
  ...
  if (
    ...
    throw new Error(
      `v8.2 KOSPI market-date lookup failed: ${kospiResponse.error.message}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `v8.2 KOSDAQ market-date lookup failed: ${kosdaqResponse.error.message}`,
    );
  }
...
export async function screenPointInTimeUniverseV82(
  ...
  if (
    ...
    throw new Error(
      `v8.2 universe definition load failed: ${definitionError.message}`,
    );
  }
  ...
  const criteria = {
    version:
      "UNIVERSE_ELIGIBILITY_LIQUIDITY_SCREEN_V8_2",
  ...
  if (
    ...
    throw new Error(
      `v8.2 screening run create failed: ${
        runError?.message ??
        "NO_RUN"
      }`,
    );
  ...
  try {
    const {
      ...
      await supabase
        .rpc(
          "compute_stock_universe_liquidity_metrics_v8_2",
          {
            p_universe_code:
              universeCode,
    ...
    if (
      ...
      throw new Error(
        `v8.2 liquidity metric RPC failed: ${metricError.message}`,
      );
    }
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          `v8.2 screening result insert failed: ${error.message}`,
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        `v8.2 screening run update failed: ${runUpdateError.message}`,
      );
    }
